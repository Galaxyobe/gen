...
func BoilerplatePath() string {
	...
	paths := []string{
		...
		filepath.Join(reflect.TypeOf(empty{}).PkgPath(), "/../../boilerplate/boilerplate.go.txt"),
		// works when run from root of module vendoring k8s.io/code-generator
		"vendor/github.com/galaxyobe/gen/boilerplate/boilerplate.go.txt",
		// works when run from root of $GOPATH containing k8s.io/kubernetes
		"github.com/galaxyobe/gen/boilerplate/boilerplate.go.txt",
	}

	// see if we can locate the module directory and add that to the list
	config := packages.Config{Mode: packages.NeedModule}
	if loadedPackages, err := packages.Load(&config, "github.com/galaxyobe/gen/pkg/util"); err == nil {
		for _, loadedPackage := range loadedPackages {
			if loadedPackage.Module != nil && loadedPackage.Module.Dir != "" {
				paths = append(paths, filepath.Join(loadedPackage.Module.Dir, "boilerplate/boilerplate.go.txt"))
			}
		}
	}
